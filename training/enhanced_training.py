...
def train_with_curriculum(model_type="DQN", total_timesteps=300000):
    """Train model with curriculum learning and adaptive hyperparameters"""
    
    print(f"🚀 Starting Enhanced Training for {model_type}")
    print("=" * 60)
    ...
    for phase_idx, phase in enumerate(phases):
        ...
        if model_type == "REINFORCE":
            ...
            checkpoint_callback = CheckpointCallback(
                save_freq=10000,
                save_path=f"models/enhanced/{model_type.lower()}/",
        ...
        if performance > best_performance:
            ...
            best_model = model
            if model_type == "REINFORCE":
                model.save(f"models/enhanced/{model_type.lower()}/best_{model_type.lower()}.pt")
            else:
                model.save(f"models/enhanced/{model_type.lower()}/best_{model_type.lower()}")
        ...
        with open(f"models/enhanced/{model_type.lower()}/phase_{phase_idx + 1}_stats.json", 'w') as f:
            json.dump(stats, f, indent=2)
    
    print(f"\n✅ Enhanced Training Complete!")
    print(f"🏆 Best Performance: {best_performance:.3f}")
    print(f"💾 Best model saved to: models/enhanced/{model_type.lower()}/best_{model_type.lower()}")
...
def create_training_report(model_type, callback):
    ...
    plt.tight_layout()
    plt.savefig(f"models/enhanced/{model_type.lower()}/training_report.png", dpi=300, bbox_inches='tight')
    plt.show()
    ...
    with open(f"models/enhanced/{model_type.lower()}/training_summary.json", 'w') as f:
        json.dump(summary, f, indent=2)
    
    print(f"📊 Training report saved to: models/enhanced/{model_type.lower()}/")

if __name__ == "__main__":
    # Create directories
    os.makedirs("models/enhanced/dqn", exist_ok=True)
    os.makedirs("models/enhanced/ppo", exist_ok=True)
    os.makedirs("models/enhanced/a2c", exist_ok=True)
    os.makedirs("models/enhanced/reinforce", exist_ok=True)
    
    # Train all models with enhanced training
    models = {}
    callbacks = {}
    
    for model_type in ["DQN", "PPO", "A2C", "REINFORCE"]:
        print(f"\n{'='*60}")
        print(f"🎯 Training {model_type} with Enhanced Curriculum Learning")
        print(f"{'='*60}")
        
        model, callback = train_with_curriculum(model_type, total_timesteps=300000)
        models[model_type] = model
        callbacks[model_type] = callback
        
        # Create training report
        create_training_report(model_type, callback)
    
    print(f"\n🎉 All Enhanced Training Complete!")
    print(f"📁 Models saved in: models/enhanced/")
    print(f"📊 Reports saved for each model")
    
    # Final comparison
    print(f"\n🏆 Final Performance Comparison:")
    for model_type, callback in callbacks.items():
        final_performance = callback.mean_rewards[-1] if callback.mean_rewards else 0
        print(f"  {model_type}: {final_performance:.3f}") 