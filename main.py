...
from environment.rendering import render_grid_dashboard, close_enhanced_rendering
...
class CustomReinforcePolicy(nn.Module):
    def __init__(self, input_size, hidden_size, num_actions):
        ...
        # Enhanced policy network (matches the saved model structure)
        ...
        # Enhanced value network (matches the saved model structure)
    ...
    def forward(self, x):
        # Get action logits
        action_logits = self.policy_net(x)
        # Get value
...
def load_reinforce_model(env, path):
    ...
    policy = CustomReinforcePolicy(
        ...
        hidden_size=128,  # Match the saved model's hidden size (enhanced)
...
def get_model_predictions(model, obs, model_name):
    ...
    if model_name == "REINFORCE":
        ...
        with torch.no_grad():
            # Get action logits from the enhanced network
            action_logits = model.policy_net(obs_tensor)
